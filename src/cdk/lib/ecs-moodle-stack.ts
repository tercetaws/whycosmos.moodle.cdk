...
export class EcsMoodleStack extends cdk.Stack {
  ...
  constructor(scope: cdk.App, id: string, props: EcsMoodleStackProps) {
    ...
    // CloudTrail
    const trailBucket = new s3.Bucket(this, 'moodle-cloudtrail-bucket', {
      blockPublicAccess: s3.BlockPublicAccess.BLOCK_ALL,
      enforceSSL: true,
      encryption: s3.BucketEncryption.S3_MANAGED
    });
    const trail = new cloudtrail.Trail(this, 'moodle-cloudtrail-trail', {
      bucket: trailBucket
    });
    ...
    const moodleDb = new rds.DatabaseInstance(this, 'moodle-db', {
      engine: rds.DatabaseInstanceEngine.mariaDb({ version: rds.MariaDbEngineVersion.VER_11_4_5}),
      vpc: vpc,
      vpcSubnets: { 
    ...
    const repository = ecr.Repository.fromRepositoryName(this, 'ecr-moodle-repository', props.moodleImageRepository);
    const moodleContainerDefinition = moodleTaskDefinition.addContainer('moodle-container', {
      ...
      environment: {
        ...
        'MOODLE_EMAIL': 'arun@whycosmos.com',
        'MOODLE_SITE_NAME': 'Why Cosmos',
